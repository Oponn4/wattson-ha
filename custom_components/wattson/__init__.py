...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def _first_real_tick(_hass: HomeAssistant) -> None:
        """Echter Zyklus, sobald HA fertig gestartet ist.

        Der Refresh oben läuft mitten im Start und ist deshalb nur lesend
        (Warmup-Zweig im Coordinator). Ohne diesen Nachschlag stünde Wattson
        bis zum nächsten Intervall — 5 Minuten — ohne Entscheidung da.
        """
        await coordinator.async_request_refresh()

    entry.async_on_unload(async_at_started(hass, _first_real_tick))
